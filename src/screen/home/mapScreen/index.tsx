...
const MapScreenComponent = forwardRef<any, any>((props: any, ref) => {
  ...
  const AmbulanceIcon =
    taxidoSettingData?.cabbooking_values?.general?.ambulance_image;
  const finalMarkerIcon =
    selfDriver?.service_id == 4 ? AmbulanceIcon : markerIcon;

  const webViewRef = useRef<WebView>(null);
  const { Google_Map_Key } = useValues();
  const { latitude: storedLat, longitude: storedLng } = useStoredLocation();
  ...
  useEffect(() => {
    setInitialLat(prev => prev === null ? storedLat : prev);
    setInitialLng(prev => prev === null ? storedLng : prev);
  }, [storedLat, storedLng]);

  const DEFAULT_LAT = initialLat;
  const DEFAULT_LNG = initialLng;
      ...
      try {
        ...
        const updateMapPeakZones = (peakZones: any[]) => {
          if (Array.isArray(peakZones) && peakZones.length > 0) {
            const allZonesCoords = peakZones
              .map((pz: any, idx: number) => {
                ...
                console.warn(
                  `[PeakZone] Zone ${idx} has no valid coordinates array:`,
                  pz,
                );
                return [];
              })
        ...
        echo
          ...
          .listen(".peak.zone.update", (data: any) => {
            if (data?.peak_zones) {
              ...
              console.warn(
                "[PeakZone] Event data missing peak_zones array:",
                data,
              );
            }
          })
          ...
          .error((err: any) => {
            console.error(
              "[MapScreen] Peak zone subscription error for",
              channelName,
              ":",
              err,
            );
      ...
      if (granted) {
        watchId = Geolocation.watchPosition(
          (position) => {
            ...
            setInitialLat(prev => prev === null ? latitude : prev);
            setInitialLng(prev => prev === null ? longitude : prev);
  ...
  <html>
    ...
    \`;
        ...
        function drawCurve(startLatLng, endLatLng){
          ...
          activeCurve=new google.maps.Polyline({path:curvedPath, geodesic:true, strokeColor:"#F0A82F", strokeOpacity:1.0, strokeWeight:2});
...
function initMap() {
  ...
  pickZones.forEach(zoneCoords => {
    ...
    polygon.on('click', () => {
      ...
      // Draw curve
      activeCurve = L.polyline([currentLatLng, nearest], { color:'#F0A82F', weight:2 }).addTo(map);
...
`;
  ...
  const showMap = DEFAULT_LAT !== null && DEFAULT_LNG !== null;
