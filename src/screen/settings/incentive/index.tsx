...
import { toLocalDateString } from '../../../utils/functions'
...
export function Incentive() {
  ...
  useEffect(() => {
    ...
    // selectedDate is already the canonical YYYY-MM-DD the server echoes back in
    // weekly.days[].date, so send it through unchanged. Re-deriving it here used
    // to drop the zero padding and could land on a different day than the one
    // the lookup below matches on.
    dispatch(incentivesValue({ incentivedate: selectedDate }))
  }, [selectedDate])
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        >
          <FlatList
            ...
            renderItem={({ item }) => {
              const formattedDate = toLocalDateString(item)
